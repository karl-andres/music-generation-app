...
import { Loader2, Music, RefreshCcw, Search, XCircle, Play, MoreHorizontal, Download, Pencil } from "lucide-react";
...
export function TrackList({ tracks }: { tracks: Track[] }) {
    ...
    const filteredTracks = tracks.filter((track) => track.title?.toLowerCase().includes(searchQuery.toLowerCase()) ?? track.prompt?.toLowerCase().includes(searchQuery.toLowerCase()))
         ...
         <div className="flex flex-1 flex-col overflow-y-auto">
            <div className="flex-1 p-6">
                <div className="mb-4 flex items-center justify-between gap-4">
                    ...
                    <Button disabled={isRefreshing} variant="outline" size="sm" onClick={handleRefresh}>{isRefreshing ? <Loader2 className="mr-2 animate-spin" /> : <RefreshCcw className="mr-2" />}Refresh</Button>
                </div>
                ...
                {filteredTracks.length > 0 ? (
                    ...
                    switch (track.status) {
                        ...
                        default:
                            return (
                                <div key={track.id} onClick={() => handleTrackSelect(track)} className="hover:bg-muted/50 flex cursor-pointer items-center gap-4 rounded-lg p-3 transition-colors">
                                    {/* Thumbnail */}
                                    ...
                                    {/* Actions */}
                                    <div className="flex items-center gap-2">
                                        <Button onClick={async (e) => {e.stopPropagation(); await setPublishedStatus(track.id, !track.published)}} variant="outline" size="sm" className={`cursor-pointer ${track.published ? "border-red-200" : ""}`}>{track.published ? "Unpublish" : "Publish"}</Button>
                                        <DropdownMenu>
            ...
            {trackToRename && (
                <RenameDialog
                    ...
                    onRename={(trackId, newTitle) => renameSong(trackToRename.id, newTitle)}
                />
            )}
         </div>
    )
